...
MODEL_PATH     = RESULTS_DIR / "vggface_best.keras"
CLASS_JSON     = RESULTS_DIR / "class_names.json"
LOG_CSV        = Path("results/pointage_log.csv")
MIN_IMAGES     = 10
IMG_SIZE       = (224, 224)
TOP_K          = 5
CONF_THRESHOLD = 0.10
...
def load_model_and_classes():
    if not MODEL_PATH.exists():
        raise FileNotFoundError(
            ...
            "lancez d'abord : python train_all.py --model vggface"
        )
    model = tf.keras.models.load_model(str(MODEL_PATH))
    return model, load_class_names()


print("Chargement du modele VGGFace...")
try:
...
def draw_result(image: Image.Image, name: str, conf: float) -> Image.Image:
    ...
    band = Image.new("RGBA", (w, bh), (26, 35, 126, 210))
...
def identify(input_image):
    """Identifie le visage et retourne l'image annotee + infos."""
...
def pointer(input_image, type_pointage: str):
    """Enregistre un pointage arrivee ou depart."""
    if input_image is None:
...
with gr.Blocks(css=CSS, title="Systeme RH — Reconnaissance Faciale") as demo:
    ...
    <div id="header">
      ...
      <p>Reconnaissance faciale par VGGFace — {MODEL_STATUS}</p>
    </div>
    """)
    ...
    # Horloge mise a jour toutes les secondes
    try:
