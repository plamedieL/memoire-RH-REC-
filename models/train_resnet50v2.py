...
Backbone ResNet50 (25M params) - 1.5x plus rapide que VGGFace.
Accuracy ciblee : 91-93% sur LFW-deepfunneled.
"""
import sys
import json
import numpy as np
...
def collect_paths():
    print("Collecte des chemins pour ResNet50...")
    paths, labels = [], []
    class_dirs = sorted(
...
def evaluate(model, val_ds):
    ...
    print("\n" + "=" * 47)
    print("  METRIQUES SUR LA VALIDATION — ResNet50")
    print("=" * 47)
...
def train():
    ...
    callbacks = [
        ModelCheckpoint(str(RESULTS_DIR / "resnet50_best.keras"),
    ...
    model.save(str(RESULTS_DIR / "resnet50_final.keras"))

    with open(RESULTS_DIR / "class_names.json", "w") as f:
        json.dump(class_names, f)

    print("\n[ResNet50] Evaluation finale...")
    ...
    print(f"[ResNet50] Termine ! results -> {RESULTS_DIR / 'results.json'}")
    return results
